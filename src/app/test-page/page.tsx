...
export default function TestPage() {
  ...
  const handleHealthCheck = async () => {
    try {
      ...
      const data = (await res.json()) as HealthData | ErrorData;
      if ("message" in data) {
        setMessage(`✅ Health Check: ${data.message}`);
      } else {
        setMessage(`❌ Health Check Error: ${data.error ?? "Unknown error"}`);
      }
    } catch (err) {
      setMessage(`❌ Health Check Error: ${String(err)}`);
    }
  };
  ...
  const handleSignup = async () => {
    try {
      ...
      const data = (await res.json()) as UserData & ErrorData;
      if (res.ok) {
        setMessage(`✅ Signup successful: ${data.username}`);
        setUserData(data);
      } else {
        setMessage(
          `❌ Signup failed: ${data.error ?? res.status.toString()}`
        );
      }
    } catch (err) {
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full bg-gray-500 hover:bg-gray-600 text-white font-semibold py-2 rounded-lg transition text-sm"
              >
                🏥 Health Check
              </button>
              <input
